---
title: Product Data with cURL and CSV
---
...
```curl
curl --user AAAXXXXXXXXXXXX: 'https://api.datafiniti.co/v3/data/products?view=products_all&format=JSON&q=categories:shoes&records=1&download=false'
```
...
```curl
curl --user AAAXXXXXXXXXXXX: 'https://api.datafiniti.co/v3/data/products?view=products_all&format=JSON&q=categories:shoes+AND+-categories:shirts+AND+categories:women+AND+%28brand%3A%2A+OR+manufacturer%3A%2A%29+AND+prices%3A%2A&records=10&download=false'
```
...
```curl
curl --user AAAXXXXXXXXXXXX: 'https://api.datafiniti.co/v3/data/products?view=products_all&format=CSV&q=categories:shoes+AND+-categories:shirts+AND+categories:women+AND+%28brand%3A%2A+OR+manufacturer%3A%2A%29+AND+prices%3A%2A&download=true'
```
...
```curl
curl 'http://datafiniti-downloads.s3.amazonaws.com/AAAXXXXXXXXXXXX/6073_1.txt?AWSAccessKeyId=AKIAIXQMCWHOZB3O35SA&Signature=2YtBsW9xY8CZrWDECcdLzyx4Jlk%3D&Expires=1484754763' > output.csv
```
...
<Image src="https://blob-cdn.documentation.ai/org-a829270c-4408-45aa-90cb-a8cad2967955/doc-82f0e261-4f6b-45af-9f00-b0b805bf07d2/1775452667873-exooo3e8zut-7938051-shoe_sample.png?fm=auto&auto=compress%2Cformat" width="1920" height="1080" alt="Sample product data output in Excel" noCaption />
