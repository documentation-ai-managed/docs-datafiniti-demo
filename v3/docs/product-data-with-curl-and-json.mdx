---
title: Product Data with cURL and JSON
---
...
```curl
curl --user AAAXXXXXXXXXXXX: 'https://api.datafiniti.co/v3/data/products?view=products_all&format=JSON&q=categories:shoes+AND+-categories:shirts+AND+categories:women+AND+%28brand%3A%2A+OR+manufacturer%3A%2A%29+AND+prices%3A%2A&download=true'
```
...
From here, you'll most likely want to process the data using the programming language of your choice. Our other [JSON guides](/v3/docs/getting-started-with-product-data) provide instructions on how to process this data.
