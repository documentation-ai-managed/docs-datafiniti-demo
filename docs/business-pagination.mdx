...
An alternative to running a business data search, you can also use the API to page through the data. This approach allows you to consume one chunk at a time. Then continue from where the end of the previous page left off. To do the we will need to modify the [Business Data](/reference/businesses) endpoint like so:

```curl
POST <https://api.datafiniti.co/v4/business/paginate?page=1&limit=500>
```
...
Please note that with pagination you can query more records at once. So be sure your logic, when using Datafiniti's API, accounts for your business data plan credit limit.
</Callout>

Note that the query syntax remains the same, so you can copy / use the same business data search queries as normal.
