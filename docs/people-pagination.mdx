...
An alternative to running a people data search, you can also use the API to page through the data. This approach allows you to consume one chunk at a time. Then continue from where the end of the previous page left off. To do the we will need to modify the [People Data](/reference/people-data) endpoint like so:
...
<Columns cols="2">
  <Card title="Find brokers in your area" href="/docs/find-brokers-in-your-area" icon="map-pin" horizontal="false">
    Locate real estate brokers operating within a specific geographic area using paginated queries.
  </Card>
</Columns>
