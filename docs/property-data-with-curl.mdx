...
<ParamField body="query" param-type="string" required="false" deprecated="false">
  `query` tells the API what you want to search.  In this case, you're telling the API you want to search by `country`.  Any property in the US will be returned.

  **Example**:"query": "country:US"
</ParamField>

<ParamField body="num_records" param-type="integer" required="false" deprecated="false">
  `num_records` tells the API how many records to return in its response.  In this case, you just want to see 1 matching record.

  **Example:**"num\_records": 1
</ParamField>

<ParamField body="format" param-type="string" required="false" deprecated="false">
  `Format` tells the API what format to output the data in the response. Can be either `CSV` or `JSON.`
...
<ResponseField name="total_cost" field-type="string" required="false" deprecated="false">
  The number of credits this request has cost you.  Property records only cost 1 credit per record.
</ResponseField>

<ResponseField name="records" field-type="string" required="false" deprecated="false">
...
Each property record will have multiple fields associated with it.  You can see a full list of available fields in our [Property Data Schema](/docs/property-data-schema).
...
```curl
curl --request POST --url https://api.datafiniti.co/v4/businesses/search --header 'authorization: Bearer AAAXXXXXXXXXXXX' --data '{"query":"country:US AND propertyType:\"Single Family Dwelling\"","format":"json","num_records": 10}'
```
...
3. It changes the `format` to `json` so we can export the data later to a specific format.

<Callout kind="info" collapsed="false">
  **Formatting your data**

  Please note that the format can be changed to `CSV` as well for ease to import into a spreadsheet
</Callout>
...
Once we like what we see from the sample matches, it's time to download the entire data set!  To do this, we're going to further modify our request to look like the following for each data format

```curl
curl --request POST --url https://api.datafiniti.co/v4/businesses/search --header 'authorization: Bearer AAAXXXXXXXXXXXX' --data '{"query":"country:US AND propertyType:\"Single Family Dwelling\"", "num_records": 50,"format":"json", "download": true}'
```
...
<Callout kind="alert" collapsed="false">
  **Num\_record not being specified**
...
<ResponseField name="ID" field-type="string" required="false" deprecated="false">
  This is a unique identifier for the request.
</ResponseField>

<ResponseField name="results" field-type="string" required="false" deprecated="false">
...
<ResponseField name="user_id" field-type="string" required="false" deprecated="false">
...
<ResponseField name="date_started" field-type="string" required="false" deprecated="false">
  The date and time the download started.
</ResponseField>

<ResponseField name="date_updated" field-type="string" required="false" deprecated="false">
  The last time the download information was updated.
</ResponseField>

<ResponseField name="num_downloaded" field-type="string" required="false" deprecated="false">
  The number of records that have been downloaded so far.
</ResponseField>

<ResponseField name="data_type" field-type="string" required="false" deprecated="false">
  The data type you queried.
</ResponseField>

<ResponseField name="query" field-type="string" required="false" deprecated="false">
  The query you ran.
</ResponseField>

<ResponseField name="format" field-type="string" required="false" deprecated="false">
  The data format you requested.
</ResponseField>

<ResponseField name="num_records" field-type="string" required="false" deprecated="false">
...
From here, you'll most likely want to process the data using the programming language of your choice. Our other [Getting Started with Property Data](/docs/getting-started-with-property-data) provide instructions on how to process this data.

### Open the CSV file(s) in Excel
...
<Image src="https://files.readme.io/b8b08d6f8ae5cdf1c77c8c56b6da899a6c006ce27fb76ddfb795aaf0a4f3b50b-image.png" width="1920" height="1080" alt="Screenshot showing open the csv file(s) in excel" />
...
For more use case specific examples please visit our [Use Cases for Property Data](/docs/use-cases-for-property-data).
...
<Callout kind="info" collapsed="false">
  **JSON Format**
  ...
  The JSON data will actually be a text file, instead of a single JSON object. Each line in the text file is a JSON object. We format the data this way because most programming languages won't handle parsing the entire data set as a JSON object with their standard system calls very well. This is commonly know as NDJSON.
</Callout>

From here, you'll most likely want to process the data using the programming language of your choice. Our other [JSON guides](/docs/getting-started-with-property-data) provide instructions on how to process this data.
