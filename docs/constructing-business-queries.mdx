---
title: Constructing Business Queries
---
...
You can do similar queries on any of the fields in the [business schema](/docs/business-data-schema).  Here's an example that queries the `name` field:
...
As an alternative or used for testing purposes you can set your `num_records` to 0 in your business search query. This is will only return the `num_found` of the query and will cost 0 credits to search.

<Card title="Try it in the API Playground" href="/reference/businesses" icon="play" horizontal={false}>
  Test your business search queries directly in the API reference.
</Card>
