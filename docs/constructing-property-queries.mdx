...
<Callout kind="info" collapsed="false">
  **Address normalization** — Datafiniti uses standardized street name normalization, converting `street` to `st`, `drive` to `dr`, and so on. If you wish to exactly match addresses to Datafiniti's property data, you will need to use this normalization method. Learn more in [Normalized Property Addresses](https://docs.datafiniti.co/docs/normalized-address-data).
</Callout>
