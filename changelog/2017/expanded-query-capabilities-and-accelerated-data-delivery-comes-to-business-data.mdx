---
title: "June 15, 2017 — Expanded Query & Performance for Business Data"
---

## June 15, 2017

In February, we announced some dramatic changes to our Product Data.  We're excited to announce that those same changes have now come to our Business Data.  You can read our main announcement here.
...
* [Getting Started with Business Data](https://datafiniti-api.readme.io/docs/getting-started-with-business-data)
...
* Some field names have changed in order to abide by a consistent naming convention.  See the last note under Data Quality below.  You can view the updated product schema [here](https://datafiniti-api.readme.io/docs/business-data-schema).
...
* You can now query on fields within multi-valued fields, except for the `reviews.text` nested field.  E.g., `q=reviews.rating:5` will return all products with at least one review that has a 5-star rating.
...
* You can now do a query like `q=reviews.rating:>1` to return all businesses with a review greater than 1.  `>`, `>=`, `<`, and `<=` are supported.  These should only be used on fields that only contain numeric values.
...
* Previously, quick successive API calls could result in dramatically different estimated\_total values, which was confusing.  Our new backend shows the same `estimated total` value each time for the same API call (barring any additions to the database).

**Standardized naming convention for all fields**

* All fields are now pascal-cased.  E.g., `dateSeen`, \`facebookPageURL'.  All multi-valued fields now use a plural word.  E.g., reviews, features.
